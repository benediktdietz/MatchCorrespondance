...
#########################
#########################
DATA_DIR = 'tensorflow_logs/doublette_test10/'
...
plt.figure()
plt.plot(np.reshape(tdf1[0,:], (-1,1)))
plt.plot(np.reshape(tdf2[0,:], (-1,1)))
plt.plot(np.reshape(tdf3[0,:], (-1,1)))
plt.show()

def feeder(log, epoch, batchsize, iterator):
    ...
    return p1, p2, p3, log, epoch, iterator
...
with tf.Session() as sess:
    ...
    while epoch <= epochs:
        ...
        data1, data2, data3, log, epoch, i = feeder(log, epoch, batch_size_train, iterator)
