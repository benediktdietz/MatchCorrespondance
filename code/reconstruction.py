...
d1 = np.load('rec_data/dist1_rec_2000.npy')
d1 = np.reshape(np.reshape(d1, (d1.shape[0], d1.shape[2])), (-1))
 
info1 = np.reshape(np.load('rec_data/info1_rec_2000.npy'), (-1))
info2 = np.reshape(np.load('rec_data/info2_rec_2000.npy'), (-1))

d1idx = np.reshape(np.asarray(np.where(d1 <= np.sort(d1)[rank_threshold])), (-1))
print('distances below threshold -->', len(d1idx))
bestidx = d1idx[np.int(np.asarray(np.where(d1[d1idx] == np.min(d1[d1idx]))))]

d1idx2nd = np.reshape(np.asarray(np.where(d1 > np.sort(d1)[rank_threshold])), (-1))
d1idx2nd = np.reshape(np.asarray(np.where(d1idx2nd <= np.sort(d1)[2*rank_threshold])), (-1))
...
for i in range(len(d1idx)):
    ...
    _, _, _, _, sigma1_rec, sigma2_rec = get_recon_data(d1idx[i])

    x_data.append(sigma1_rec)
    y_data.append(sigma2_rec)

x_data = np.reshape(np.asarray(x_data), (-1,3))
y_data = np.reshape(np.asarray(y_data), (-1,3))
...
# l2_loss = lambda x: np.mean(((np.cos(x[0]) * x_data[:,2] + np.sin(x[0]) * x_data[:,0] + x[1] - y_data[:,2]) ** 2) + ((-1 * (np.sin(x[0]) * x_data[:,2] + np.cos(x[0]) * x_data[:,0] + x[2]) + y_data[:,0]) ** 2) + ((x_data[:,1] + x[3] - y_data[:,1]) ** 2))

# # l2_loss = lambda x: np.linalg.norm(np.dot(np.array([[np.cos(x[0]), -np.sin(x[0]), 0], [np.sin(x[0]), np.cos(x[0]), 0.], [1., 1., 0.]]), np.reshape(np.array([[x_data[:,2]], [-x_data[:,0]], [x_data[:,1]]]), (3,-1))) + np.array([[x[1]], [x[2]], [x[3]]]) - np.array([[y_data[:,2]], [-y_data[:,0]], [y_data[:,1]]]))


# res = minimize(squareLoss, [1e-3, 1e-3, 1e-3, 1e-3, 1e-3])
