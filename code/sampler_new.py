...
#########################
#########################
DATA_DIR = 'tensorflow_logs/doublette_test8/'
...
def getPatchData_CPU(pointData,voxelGridPatchRadius,voxelSize,voxelMargin):
    ...
    while non_match_found < 1:
        ...
        while point2point_dist < point_dist_threshold:
            ...
            if np.array(np.nonzero(depthImg3[frame_size:-frame_size,frame_size:-frame_size])).shape[1] > 10:
                ...
                while np.absolute(cy - row3) > (cy - frame_size) or np.absolute(cx - col3) > (cx - frame_size):
...
def sampleTDFs(snap1):
    ...
    non0pix1 = np.array(np.nonzero(depthImg1))


    # if non0pix1.shape[1] < 10:

    #     print('no tree in image')

    #     return 0

    print('shape of nonzero pixels:', non0pix1.shape)
    print()


    if non0pix1.shape[1] > max_samples:

        non0pix1 = non0pix1[:, np.random.choice(np.arange(non0pix1.shape[1]), max_samples, replace=False)]
    ...
    for j in range(int(non0pix1.shape[1])):

        # if j % 10000 == 0:
        #     print('loop at iteration ', j)
        b = 100*j/non0pix1.shape[1]
        sys.stdout.write("\r%d%%" % b)
        sys.stdout.flush()

        rand_row, rand_col = non0pix1[:,j]
        ...
        PointRealWorld1 = drone1position + quaternion_cam1.rotate(np.array([gamma1 + .46, -alpha1, beta1]))
        ...
        p1_info = {'framePath': random_tree_path + file_index1,
                   ...
                   'camCoords': sigma1[:3],
                   'cam_position': cam_position1,
                   'PointRealWorld': PointRealWorld1}
        ...
        samples.append(p1_voxelGridTDF)
        samples_meta.append(p1_info)


    samples = np.reshape(np.asarray(samples), (-1, window_size, window_size, window_size))
    samples_meta = np.reshape(np.asarray(samples_meta), (-1))


    np.save('/media/drzadmin/DATA/recon_files/tdfs_new_reconstruction_' + str(snap1) + '.npy', samples)
    np.save('/media/drzadmin/DATA/recon_files/meta_new_reconstruction_' + str(snap1) + '.npy', samples_meta)

    print()
    print('tdf sampling of frame no', snap1, '--> successful!')
...
# print()
# print('time per tdf ------> ', np.round(time_elapsed/max_samples, 3))
# print('time per frame ----> ', np.round(time_elapsed/max_samples*40000))
