...
def TDF_recon_loader_csv(batchsize):
    ...
    while counter < int(batchsize):
        ...
        if np.mean(np.reshape(p1, (-1))) != 0. and np.mean(np.reshape(p2, (-1))) != 0. and np.mean(np.reshape(p1, (-1))) != 0.:
            ...
            if counter == 0:
                p1data = pd.DataFrame(list(p1_info))
                p2data = pd.DataFrame(list(p2_info))
                p3data = pd.DataFrame(list(p3_info))

            if counter > 0:
                p1data.append(pd.DataFrame(list(p1_info)), ignore_index=True)
                p2data.append(pd.DataFrame(list(p2_info)), ignore_index=True)
                p3data.append(pd.DataFrame(list(p3_info)), ignore_index=True)
...
for i in range(1):
    ...
    tdf1, p1data, tdf2, p2data, tdf3, p3data = TDF_recon_loader(1000)

    print()
    print('batch number ' + str(i) + ' loaded')
    print(tdf1.shape)

    np.save('tdf1_rec' + str(i) + '.npy', tdf1)
    np.save('tdf2_rec' + str(i) + '.npy', tdf2)
    np.save('tdf3_rec' + str(i) + '.npy', tdf3)

    np.save('p1inf_rec' + str(i) + '.npy', p1data)
    np.save('p2inf_rec' + str(i) + '.npy', p2data)
    np.save('p3inf_rec' + str(i) + '.npy', p3data)

    tdf1, p1data, tdf2, p2data, tdf3, p3data = [], [], [], [], [], []
