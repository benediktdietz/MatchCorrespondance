...
#########################
#########################
DATA_DIR = 'tensorflow_logs/doublette_test9/'
...
recpath = '/media/drzadmin/DATA/recon_files/'
...
print('load initial tdfs')
tdf1 = np.load(path + 'tdf11.npy')
tdf2 = np.load(path + 'tdf21.npy')
tdf3 = np.load(path + 'tdf31.npy')

print('load validation tdfs')
tdf1val = np.load(path + 'tdf1val.npy')
tdf2val = np.load(path + 'tdf2val.npy')
tdf3val = np.load(path + 'tdf3val.npy')

tdf_size = tdf1.shape[0]
...
with tf.Session(config=config) as sess:
    ...
    for i in range(0,iterations):
        ...
        data1, data2, data3, logTDF, logFile, epoch, tdf1, tdf2, tdf3 = feeder(logTDF, logFile, epoch, batch_size_train, tdf1, tdf2, tdf3)

        # data1, data2, data3, log, epoch = feeder(log, epoch, batch_size_train)


        summary, _ , loss_train, l2md, l2nonmd, ML, NML, o1, o2, o3 = sess.run([
            merged, 
            train_step, 
            loss, 
            l2diff_match, 
        ...
        if i % 100 == 0:
            ...
            saver.save(sess, os.path.join(DATA_DIR, "treematch.ckpt"), i)
        ...
        if i % 50 == 0:
            ...
            print('worst', batch_size, ' samples used to train,  epoch:', epoch, ' (', int(numfiles*tdf_size), 'triplets)')
            print('###########################################################################')
        ...
        if i % acc_print_freq == 0:
            ...
            logVal = list(np.arange(tdf_size))

            for h in range(int(np.round(tdf_size/batch_size_train))):

                progress = h/int(np.round(tdf_size/batch_size_train))*100
                progress = np.round(progress, 1)

                sys.stdout.write('sampling progress --->' + "\r%d%%" % progress)
                sys.stdout.flush()
                ...
                data1_test, data2_test, data3_test, logVal = feeder_val(batch_size_train, logVal)
                
                loss_val, l2diff_match_val, l2diff_non_match_val, distances1_test, distances2_test = sess.run([loss, match_loss, non_match_loss, distances1, distances2], feed_dict={
                x1: data1_test,
                x2: data2_test,
                x3: data3_test})
                ...
                storage_dist.append(np.array([loss_val, l2diff_match_val, l2diff_non_match_val]))
            ...
            storage_dist = np.mean(np.asarray(storage_dist), 0)

            runData = np.append(storage_match_distances, storage_non_match_distances)

            filename_dists = DATA_DIR + 'runData_' + str(i) + '.npy'
            filename_loss = DATA_DIR + 'runData_loss_' + str(i) + '.npy'
            print()
            print('saving distances to ', filename_dists)
            np.save(filename_dists, runData)
            np.save(filename_loss, storage_dist)
            print()
            runData = []


            ####################################
            data = np.reshape(np.load(filename_dists), (1,-1))

            num_points = data.shape[1]
            num_iters = .5 * num_points / batch_size_train

            matches = np.reshape(data[0,:int(.5*num_points)], (1,-1))
            non_matches = np.reshape(data[0,int(.5*num_points):], (1,-1))


            leaves_index0 = np.arange(int(.5*batch_size_train))
            leaves_index = []
            non_leaves_index = []

            for i in range(int(num_iters)):

                leaves_index.append(leaves_index0 + i * int(batch_size_train))
                non_leaves_index.append(leaves_index0 + int((i+.5) * batch_size_train))
            ...
            print()
            print('error rate ------------>', err95)
            print('val_loss -------------->', loss_val)
            print('val_match_loss -------->', l2diff_match_val)
            print('val_non_match_loss ---->', l2diff_non_match_val)
            print()
        ...
        if err95 < .5:
            ...
            print()
            print('run reconstruction data acquisition...')
            print()

            

            tdf1_rec = np.load(recpath + 'tdfs_new_reconstruction_5.npy')
            p1_info = np.load(recpath + 'meta_new_reconstruction_5.npy')
            
            tdf2_rec = np.load(recpath + 'tdfs_new_reconstruction_6.npy')
            p2_info = np.load(recpath + 'meta_new_reconstruction_6.npy')
            ...
            for k in range(rec_num):
                ...
                if k == 0:
                    ...
                    dist_store, p1_store, p2_store = [], [], []


                rec_sample_idx1 = np.random.choice(np.arange(tdf1_rec.shape[0]), batch_size_train, replace=False)
                rec_sample_idx2 = np.random.choice(np.arange(tdf2_rec.shape[0]), batch_size_train, replace=False)

                p1 = np.reshape(tdf1_rec[rec_sample_idx1,:], (batch_size_train, window_size, window_size, window_size, 1))
                p2 = np.reshape(tdf2_rec[rec_sample_idx2,:], (batch_size_train, window_size, window_size, window_size, 1))

                if k == 0:
                    p3 = p2

                for p in range(batch_size_train):

                    p1_store.append(p1_info[rec_sample_idx1[p]].get('pixelCoords'))
                    p2_store.append(p2_info[rec_sample_idx2[p]].get('pixelCoords'))


                # p1, info1, p2, info2, p3, info3, logRec = feeder_recon(batch_size_train, logRec, combos)

                distances1_rec, d2r = sess.run([distances1, distances2], feed_dict={
                x1: p1,
                x2: p2,
                x3: p3})


                dist_store.append(distances1_rec)
            ...
            print('saving results to ', DATA_DIR + 'recdata/')
            np.save(DATA_DIR + 'dist1_rec_' + str(i) + '.npy', np.asarray(dist_store))
            # np.save(DATA_DIR + 'dist2_rec_' + str(i) + '.npy', np.asarray(d2_store))

            np.save(DATA_DIR + 'info1_rec_' + str(i) + '.npy', np.asarray(p1_store))
            np.save(DATA_DIR + 'info2_rec_' + str(i) + '.npy', np.asarray(p2_store))
            # np.save(DATA_DIR + 'info3_rec_' + str(i) + '.npy', np.asarray(inf3_store))


            dist_store, p1_store, p2_store, tdf1_rec, tdf2_rec, p1_info, p2_info = [], [], [], [], [], [], []
