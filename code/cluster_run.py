...
for i in range(numfiles):
   ...
   if i == 0:
       ...
       print('load initial tdf1')
       tdf1 = np.load(path + 'tdf1' + str(i) + '.npy')
       print('load initial tdf2')
       tdf2 = np.load(path + 'tdf2' + str(i) + '.npy')
       print('load initial tdf3')
       tdf3 = np.load(path + 'tdf3' + str(i) + '.npy')

   else:

       print('load tdf1 number ' + str(i+1))
       tdf1 = np.append(tdf1, np.load(path + 'tdf1' + str(i) + '.npy'), axis=0)
       print('load tdf2 number ' + str(i+1))
       tdf2 = np.append(tdf2, np.load(path + 'tdf2' + str(i) + '.npy'), axis=0)
       print('load tdf3 number ' + str(i+1))
       tdf3 = np.append(tdf3, np.load(path + 'tdf3' + str(i) + '.npy'), axis=0)
...
def TDF_train_loader(batchsize):
   ...
   while loggerlen > 0:
       ...
       logger = np.asarray(np.where(np.isnan(tdf1_train).astype(np.int32) == 1))[0,:]
       logger = np.append(logger, np.asarray(np.where(np.isnan(tdf2_train).astype(np.int32) == 1))[0,:])
       logger = np.append(logger, np.asarray(np.where(np.isnan(tdf3_train).astype(np.int32) == 1))[0,:])
...
def feeder(log, epoch, batchsize):
   ...
   while loggerlen > 0:
       ...
       if len(log) <= batchsize:
           ...
           log = list(np.arange(tdf1.shape[0]))

           epoch += 1

       rand_samples = np.random.randint(0, len(log), size=batchsize)
   ...
   for j in range(len(rand_samples)):
       ...
       log.pop(np.sort(rand_samples)[-j])

   return p1, p2, p3, log, epoch
...
def contrastive_loss(output_1, output_2, output_3):
   ...
   with tf.name_scope("contrastive_loss"):
       ...
       with tf.name_scope('euclidean_distances'):
           ...
           distances1 = tf.reshape(distances1, [1,-1])
           ...
           distances2 = tf.reshape(distances2, [1,-1])
...
with tf.Session() as sess:
   ...
   with tf.control_dependencies(update_ops):
       ...
       train_step = tf.train.GradientDescentOptimizer(1e-03).minimize(loss)
   ...
   for i in range(0,iterations):
       ...
       if i % 1 == 0:
           ...
           print('worst', batch_size, ' samples used to train,  epoch:', epoch)
           print('###########################################################################')
