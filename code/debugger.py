...
#########################
#########################
DATA_DIR = 'tensorflow_logs/doublette_test50/'
...
def TDF_train_loader(batchsize):
    ...
    while loggerlen > 0:
        ...
        randoms = np.random.randint(0, tdf1.shape[0], batch_size_train)

        tdf1_train = tdf1[randoms,:,:,:,:]
        tdf2_train = tdf2[randoms,:,:,:,:]
        tdf3_train = tdf3[randoms,:,:,:,:]
...
def feeder(logTDF, logFile, epoch, batchsize, tdf1, tdf2, tdf3):
    ...
    if len(logTDF) <= batchsize:
        ...
        if len(logFile) == 0:
            ...
            logFile = list(np.arange(numfiles+1)).pop(0)
...
def contrastive_loss(output_1, output_2, output_3):
    ...
    with tf.name_scope("contrastive_loss"):
        ...
        with tf.name_scope('euclidean_distances'):
            ...
            l2diff_non_match = tf.case([(tf.less(l2diff_non_match1, 1e-04), f2)], default=f1)

            l2diff_match = tf.reduce_mean(values_matches)


            l2diff_match = l2diff_match * 100
            l2diff_non_match = l2diff_non_match * 100
...
with tf.Session() as sess:
    ...
    for i in range(0,iterations):
        ...
        data1, data2, data3, logTDF, logFile, epoch, tdf1, tdf2, tdf3 = feeder(logTDF, logFile, epoch, batchsize, tdf1, tdf2, tdf3)
        ...
        if i % 1 == 0:
            ...
            print('worst', batch_size, ' samples used to train,  epoch:', epoch, ' (', int(numfiles*tdf_size), 'samples)')
            print('###########################################################################')
