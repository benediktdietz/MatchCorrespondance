...
with tf.Session(config = config) as sess:    
    saver = tf.train.import_meta_graph(DATA_DIR + 'treematch.ckpt-0.meta')
    saver.restore(sess,tf.train.latest_checkpoint(DATA_DIR))

    graph = tf.get_default_graph()

    loss = graph.get_tensor_by_name('contrastive_loss')
    ...
    loss_train, l2md, l2nonmd, ML, NML, o1, o2, o3 = sess.run([
        loss, 
        l2diff_match, 
        l2diff_non_match,
        match_loss,
